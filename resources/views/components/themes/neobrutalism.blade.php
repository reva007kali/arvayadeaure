...
@slot('head')
    ...
    <link href="https://fonts.googleapis.com/css2?family=Archivo+Black&family=Lexend+Mega:wght@300;700&family=Space+Mono:ital,wght@0,400;0,700;1,400&display=swap" rel="stylesheet">
@endslot
...
<style>
    ...
    .font-sub { font-family: var(--f-sub); }
    .font-mono { font-family: var(--f-mono); }
    .theme-bg { background: var(--c-pink) }
    /* Brutalism Utilities */
    .brutal-box {
...
{{-- ======================================================================= --}}
<div id="loading-overlay" class="fixed inset-0 z-[9999] bg-[#4ECDC4] flex flex-col items-center justify-center transition-all duration-500">
    
    <!-- Retro Window -->
    <div class="bg-white border-[3px] border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] p-0 w-[300px] max-w-full">
        <!-- Title Bar -->
        ...
        <div class="p-8 flex flex-col items-center">
            ...
            <div class="w-full h-6 border-[3px] border-black p-1 bg-white relative">
                <div id="progress-fill" class="h-full bg-[#FFE66D] w-0 transition-all duration-[1500ms] border-r-[2px] border-black"></div>
            </div>
...
<script>
    ...
    window.addEventListener('load', function() {
        ...
        setTimeout(() => {
            document.getElementById('loading-text').innerText = "READY TO ROCK!";
            document.getElementById('open-invitation-btn').classList.remove('hidden');
        }, 1500);
...
<div class="relative bg-[#FFFDF5] min-h-screen overflow-x-hidden">
    ...
    @if (!empty($theme['music_url']))
        <div x-data="youtubePlayer('{{ $theme['music_url'] }}')" x-init="initPlayer()" @play-music.window="playMusic()" class="fixed bottom-6 right-6 z-50">
            <button @click="togglePlay" class="w-14 h-14 bg-[#FF6B6B] border-[3px] border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex items-center justify-center hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-none transition-all">
        ...
        <script>
            function youtubePlayer(url) {
                return { isPlaying: false, togglePlay() { this.isPlaying = !this.isPlaying; }, playMusic() { this.isPlaying = true; } }
            }
        </script>
    @endif
    ...
    <section class="min-h-screen relative flex flex-col pt-10">
        ...
        <div class="bg-[#FFE66D] border-y-[3px] border-black overflow-hidden py-2 relative z-10 rotate-1 scale-105">
            <div class="whitespace-nowrap animate-marquee font-head text-2xl text-black">
                JUST MARRIED • SAVE THE DATE • PARTY TIME • NO BORING STUFF • {{ strtoupper($groom['nickname']) }} & {{ strtoupper($bride['nickname']) }} • 
                JUST MARRIED • SAVE THE DATE • PARTY TIME • NO BORING STUFF • {{ strtoupper($groom['nickname']) }} & {{ strtoupper($bride['nickname']) }} •
            </div>
        </div>
        ...
        <div class="flex-1 flex flex-col justify-center items-center px-4 relative z-10">
            ...
            <div class="bg-white border-[4px] border-black shadow-[10px_10px_0px_0px_#4ECDC4] p-6 md:p-12 text-center max-w-2xl transform hover:-rotate-1 transition-transform duration-300">
                ...
                <div class="mt-6 flex flex-wrap justify-center gap-4">
                    <div class="bg-[#FFE66D] border-[3px] border-black px-6 py-2 font-bold font-sub shadow-[4px_4px_0px_0px_black]">
                        {{ strtoupper($groom['nickname']) }}
                    </div>
                    <div class="font-head text-4xl">&</div>
                    <div class="bg-[#4ECDC4] border-[3px] border-black px-6 py-2 font-bold font-sub shadow-[4px_4px_0px_0px_black]">
        ...
        @if ($guest)
            <div class="py-10 text-center relative z-10">
                <div class="inline-block bg-white border-[3px] border-black shadow-[6px_6px_0px_0px_#FF6B6B] px-8 py-4 rotate-2">
    ...
    <section class="py-20 px-4 bg-[#FF6B6B] border-t-[4px] border-black relative overflow-hidden">
        ...
        <div class="absolute bottom-10 right-10 w-32 h-32 bg-[#FFE66D] rounded-full border-[3px] border-black z-0"></div>

        <div class="max-w-6xl mx-auto relative z-10">
            <h2 class="font-head text-5xl text-white text-stroke-black mb-12 text-center drop-shadow-[4px_4px_0px_black]">THE PLAYERS</h2>
            ...
            <div class="flex flex-col md:flex-row gap-8 justify-center items-stretch">
                ...
                <div class="brutal-box bg-white p-4 flex-1 max-w-md mx-auto rotate-[-2deg] hover:rotate-0 hover:z-20">
                    <div class="aspect-square bg-gray-200 border-[3px] border-black mb-4 overflow-hidden relative">
                        <img src="{{ asset($groomImage) }}" class="w-full h-full object-cover grayscale contrast-125 hover:grayscale-0 transition-all duration-300">
                        <div class="absolute bottom-0 left-0 bg-[#4ECDC4] border-t-[3px] border-r-[3px] border-black px-3 py-1 font-bold font-sub text-sm">
                            GROOM
                        </div>
                    </div>
                    ...
                    <a href="https://instagram.com/{{ $groom['instagram'] }}" class="mt-4 block bg-black text-white text-center py-2 font-mono hover:bg-[#FFE66D] hover:text-black transition-colors">
                        @ {{ $groom['instagram'] }}
                    </a>
                </div>
                ...
                <div class="brutal-box bg-white p-4 flex-1 max-w-md mx-auto rotate-[2deg] hover:rotate-0 hover:z-20">
                    <div class="aspect-square bg-gray-200 border-[3px] border-black mb-4 overflow-hidden relative">
                        <img src="{{ asset($brideImage) }}" class="w-full h-full object-cover grayscale contrast-125 hover:grayscale-0 transition-all duration-300">
                        <div class="absolute bottom-0 right-0 bg-[#FF6B6B] border-t-[3px] border-l-[3px] border-black px-3 py-1 font-bold font-sub text-sm text-white">
                            BRIDE
                        </div>
                    </div>
                    ...
                    <a href="https://instagram.com/{{ $bride['instagram'] }}" class="mt-4 block bg-black text-white text-center py-2 font-mono hover:bg-[#FFE66D] hover:text-black transition-colors">
                        @ {{ $bride['instagram'] }}
                    </a>
                </div>
            </div>
    ...
    <section class="py-20 px-4 bg-[#4ECDC4] border-t-[4px] border-black">
        <div class="max-w-4xl mx-auto">
            <div class="bg-white border-[4px] border-black shadow-[12px_12px_0px_0px_#1A1A1A]">
                <!-- Window Header -->
                ...
                <div class="p-6 md:p-12 text-center">
                    ...
                    <div class="font-mono text-lg mb-8">
                        <p class="bg-[#FFE66D] inline-block px-2 border border-black transform -rotate-1">
                            TIME: {{ \Carbon\Carbon::parse($invitation->event_data[0]['date'])->format('H:i') }} WIB - DROPPING BODIES
                        </p>
                        <div class="mt-4">
    ...
    @if (!empty($moments))
        <section class="py-20 px-4 bg-white border-t-[4px] border-black overflow-hidden">
            <div class="max-w-6xl mx-auto text-center mb-12">
                <h2 class="font-head text-5xl bg-[#FFE66D] inline-block px-4 border-[3px] border-black shadow-[4px_4px_0px_0px_black] transform -rotate-1">
                    EVIDENCE
                </h2>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8 p-4">
                @foreach ($moments as $index => $photo)
                    ...
                    <div class="bg-white p-3 border-[3px] border-black shadow-[6px_6px_0px_0px_black] transform rotate-[{{ $rot }}deg] hover:rotate-0 hover:scale-105 transition-transform duration-300" data-anim="fade-up">
                        <div class="aspect-[4/5] overflow-hidden border-[2px] border-black grayscale hover:grayscale-0 transition-all">
    ...
    @if (!empty($gifts))
        <section class="py-20 px-6 bg-[#FFE66D] border-t-[4px] border-black">
            <div class="max-w-3xl mx-auto text-center">
                ...
                <p class="font-mono text-sm mb-10 bg-white inline-block border-[2px] border-black px-2">aka MONEY (Optional but cool)</p>

                <div class="flex flex-col gap-6 items-center">
                    @foreach ($gifts as $gift)
                        <!-- Brutal Credit Card -->
                        <div class="relative w-full max-w-[400px] h-[220px] bg-[#FF6B6B] border-[4px] border-black rounded-xl shadow-[8px_8px_0px_0px_black] p-6 text-white overflow-hidden group hover:translate-x-[-4px] hover:translate-y-[-4px] hover:shadow-[12px_12px_0px_0px_black] transition-all" data-anim="flip-up">
                            
                            <!-- Card Decoration -->
                            <div class="absolute top-0 right-0 w-20 h-20 bg-[#4ECDC4] rounded-bl-full border-b-[3px] border-l-[3px] border-black z-0"></div>
                            
                            <div class="relative z-10 flex flex-col h-full justify-between">
                                <div class="flex justify-between items-start">
                                    <span class="font-head text-2xl tracking-tighter">{{ $gift['bank_name'] }}</span>
                                    <div class="w-10 h-10 border-[3px] border-white rounded-full flex items-center justify-center">
                                ...
                                <div class="flex justify-between items-end font-mono text-sm">
                                    ...
                                    <div class="bg-black text-[#FFE66D] px-2 py-1 text-xs font-bold border border-white">DEBIT</div>
                                </div>
                            </div>
                        </div>
                    @endforeach
    ...
    <section class="py-20 px-4 bg-[#FFFDF5] border-t-[4px] border-black">
        <div class="max-w-2xl mx-auto">
            <div class="bg-white border-[4px] border-black p-6 md:p-10 shadow-[10px_10px_0px_0px_#FF6B6B]">
                <h2 class="font-head text-4xl mb-8 text-center bg-black text-white py-2 transform -rotate-2">ARE YOU COMING?</h2>
                ...
                <div class="border-t-[4px] border-dashed border-black pt-8">
                    <h2 class="font-head text-3xl mb-6 text-center">DROP A WISH</h2>
                    <div class="bg-[#4ECDC4] p-4 border-[3px] border-black">
    ...
    <footer class="bg-black text-white py-12 border-t-[4px] border-white text-center font-mono">
        ...
        <div class="mt-8">
            <a href="https://arvayadeaure.com" class="inline-block border border-white px-4 py-1 hover:bg-white hover:text-black transition-colors text-xs">
